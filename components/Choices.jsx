...
class Choices extends React.Component {
  constructor(props) {
    ...
    this.engine = new Engine(
      clone(this.props.nodes),
      clone(this.props.rootNode)
    );
    this.handleCardClicked = this.handleCardClicked.bind(
      this
    );
    this.handleAddToBlacklist = this.handleAddToBlacklist.bind(
      this
    );
    this.handleUndoClicked = this.handleUndoClicked.bind(
      this
    );
  ...
  getNextChoice() {
    if (this.engine.getEnded()) {
      this.props.setSortedRootNode(
        this.engine.getRootNode()
      );
      return;
    }
    this.setState(
    ...
    return Math.round(
      100 *
        (this.props.nodes.length -
          this.engine.getListNodes().length -
          1) /
        this.props.nodes.length
    );
  }
  ...
  render() {
    if (
      this.state.leftCard == null ||
      this.state.rightCard == null
    ) {
    ...
    return (
      <div id="second_div">
        <div className="container__prioritization-status">
          <div className="text__prioritization-status">
            Prioritization progress
          </div>
          <div
            className={'progressive-bar__status-structure'}
          >
            <div
              className={'progressive-bar__status'}
              role="progressbar"
        ...
        <div className="container__choose-card">
          <div className="choose-card__heading">
            Select the highest priority card
          </div>
          <Card
            id="left_button"
            side="node"
            handleClick={this.handleCardClicked}
            forget={this.handleAddToBlacklist}
            data={this.state.leftCard.value}
          />
          <Card
            id="right_button"
            side="compareNode"
            handleClick={this.handleCardClicked}
            forget={this.handleAddToBlacklist}
            data={this.state.rightCard.value}
          />
          ...
          >
            <div className="undo__button">
              <div className="undo__icon">
                <img
                  src="assets/icons/undo-icon.svg"
                  alt=""
                />
                Undo choice
              </div>
